...
RD.RIND||'~'||RD.ORDINE||'~'||SUM(ROWNUM) AS NR_ROW,
   ...
   CIS2.NVAL(SUM(CASE WHEN D.CAPITOL IN (1016) AND (D.PERIOADA=:PPERIOADA)       THEN D.COL1 ELSE NULL END)) AS COL1,
   CIS2.NVAL(SUM(CASE WHEN D.CAPITOL IN (1016) AND (D.PERIOADA=:PPERIOADA-1)     THEN D.COL1 ELSE NULL END)) AS COL2
...
INNER JOIN 
  ...
  ON TT.CUIIO=D.CUIIO  AND TT.CUIIO_VERS  = D.CUIIO_VERS 
  
WHERE
  D.FORM IN (20)           AND 
  D.FORM_VERS = :PFORM_VERS   AND
 (D.PERIOADA IN(:pPERIOADA,:pPERIOADA-1))   AND     
 (D.CAPITOL IN (1016)  AND D.RIND NOT IN ('-')) AND 
